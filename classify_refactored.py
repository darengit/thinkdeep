...
# create an html page with 2 stacked plots:
# that share an x-axis (date)
# top plot: SPX plot in OHLC format but with only spx_high, spx_low, spx_close
# bottom plot: bar plot probabilities based on a threshold of 0.6
...
def plot_results(combined: pd.DataFrame, N: int = 100, outpath: str = "spx_and_prob_stacked.html"):
    ...
    fig = make_subplots(rows=2, cols=1, shared_xaxes=True,
                        ...
                        subplot_titles=("S&P 500 Index (SPX)", "Predicted Probabilities"))

    # Top plot: SPX OHLC
    fig.add_trace(go.Candlestick(
        x=combined.index,
        open=combined["spx_close"],
    ...
    for idx, row in combined.iterrows():
        if pd.isna(row["P(1)"]) or pd.isna(row["P(0)"]):
            continue
        if row["P(1)"] >= 0.6:
            color = 'lightgreen' if row["is_training"] else 'green'
            fig.add_trace(go.Bar(x=[idx], y=[row["P(1)"]], name="P(1)", marker_color=color))
        elif row["P(0)"] >= 0.6:
            color = 'lightcoral' if row["is_training"] else 'red'
            fig.add_trace(go.Bar(x=[idx], y=[row["P(0)"]], name="P(0)", marker_color=color))

    fig.update_layout(height=800, width=1000, title_text="SPX and Predicted Probabilities")
    fig.update_xaxes(rangeslider_visible=False)

    # Show last N days by default
    if len(combined) > N:
        fig.update_xaxes(range=[combined.index[-N], combined.index[-1]])
